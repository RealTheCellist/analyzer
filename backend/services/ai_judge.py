...
Fundamental Score: {fundamental['score']}/20
Technical Score: {technical['score']}/20
Sentiment Score: {news['sentiment_score']}/5
News Score: {news['news_score']}/5

Key Fundamentals: {indicators_summary}
Technical Signals: {tech_summary}
News Summary: {news['summary']}

Based on this data, rate the investment suitability from 0 to 50.
Be balanced — 15 is average, 22+ means above average, 28+ means excellent.
...
async def _ollama_fallback(prompt: str) -> dict:
    try:
        ...
        async with httpx.AsyncClient(timeout=60.0) as client:
            ...
            score = float(parsed.get("score", 25))
            score = max(0.0, min(30.0, score))
            return {
                "score": score,
                "reasoning": parsed.get("reasoning", ""),
